...
function openFlyout(group) {
  ...
  container.innerHTML = cfg.items.map(item => `
    <a href="#" class="nav-item${AppState.currentPage === item.page ? ' active' : ''}" data-page="${item.page}">
      <span class="nav-icon">${fIcon(item.icon, 16)}</span>
      <span class="nav-label">${item.label}</span>
    </a>
  `).join('');

  // Attach click handlers to flyout nav items
...
function updateGroupActiveState(page) {
  const salesPages = ['pos', 'pos-products', 'pos-payment-methods', 'pos-report'];
  const group = salesPages.includes(page) ? 'sales' : 'accounting';
  document.querySelectorAll('.nav-group-btn').forEach(b => b.classList.remove('active-group'));
